...
def sample_action_bc(
    ...
    # observations[1:, 7 : 7 + 2] = [0.03970391, -0.03982955]
...
class MIMICDataset(seq_d.SequenceDataset):
    ...
    def __init__(
        ...
        for d in range(len(file_dataset.demos)):
            ...
            list_dict["obs"].append(
                np_obs[: done_index + 1, :-7]
            )  # delete the last 7 dim
        ...
        print("===== observations_segmented shape", self.observations_segmented.shape)
        ...
        # print("final RTG's:", self.values_raw[:400])
...
def evaluate(
    ...
    for i in pbar:
        ...
        while not done and eval_steps < max_steps:
            ...
            # print("object", new_observation[9:12])
            if reward > 0.0:
                done = True
            # print("reward", reward)
            env.render()
...
class OldGymWrapper(Wrapper, gym.Env):
    ...
    def __init__(self, env, keys=None):
        ...
        self.obs_dim = flat_ob.size - 7  # remove the last 7 dim
    ...
    def _flatten_obs(self, obs_dict, verbose=False):
        ...
        return np.concatenate(ob_lst)[:-7]  # remove the last 7 dim
